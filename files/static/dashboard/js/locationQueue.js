  ...
  var endpoint = "/api/v1/createappointment";
  var redirect = "/donor/";
...
//   };
  ...
  $('.sendAppointment').on('submit', function(e){
    var xDate = $('#timepicker').val()
    var xState = $('#state').val()
    var xLga = $('#lga').val()
    var xHospital = $('#hospital').val()
    e.preventDefault()

    var newD = new Date(xDate)
    var dTime = newD.getTime()
    var selectDate, yr = newD.getFullYear(), mnth = newD.getMonth()+1, day = newD.getDate(),hour = newD.getHours(), minu = newD.getMinutes(), sec = newD.getSeconds(); 
    if (mnth.toString().startsWith('0') || mnth > 9){
      mnth = mnth.toString()
    } else {
      mnth = '0'+ mnth.toString()
    }
    var lenHour = hour.toString().length,lenMin = minu.toString().length,lenSec = sec.toString().length;
    if(lenHour == '1'){
    ...
    if (minu.toString().startsWith('0') || minu > 9){
      minu = minu.toString()
    } else {
      minu = '0'+ minu.toString()
    }
    selectDate = day.toString()+'_'+mnth+'_'+yr.toString()+')('+hour.toString()+':'+minu.toString()+':'+sec.toString()
    $.ajax({
        type: "GET",
        url: URL+'/api/v1/validate/'+selectDate,
      ...
      // use this show date and time
      // selectDate = mnth+'_'+day.toString()+'_'+yr.toString()+' '+hour.toString()+':'+minu.toString()+':'+sec.toString()
  ...
  $('#hospital').on('change', function(){
    ...
    $('#lga').change(function(){
      var lga = $(this).children("option:selected").val()
      $.ajax({
        type: "GET",
        url: URL+'/api/v1/locate/hospital/'+lga,
        success: function (data) {
            if(data.status == 200){
              $("#hospital").empty();
              $.each( data.data, function( i, val ) {
                $("#hospital").append('<option value="'+val+'">'+ val+'</option>');
              });
              $("#hospital").prepend('<option disabled value="None" selected>--Select Hospital--</option>');
            } else {
              $('#submit').css('visibility', 'hidden')
              $('#hospital').empty().html('<option disabled selected>-- '+ data.data['hospital']+' --</option>')
              Swal.fire(
                'Oops!',
                data.data['hospital'],
                'warning'
