/*-------------------------------------------------------------------*/
/* Project: CodeUp Blood Bank - Blood Donation Campaign & Life Saver */
/* Ver: 1.0.2:B-478*/
/* Date: 23-01-2021*/
/* Author: WittyHub[TetelaINV]*/
/*-------------------------------------------------------------------*/
...
jQuery(function ($) {
    ...
    // $('#your_phone').on('input', function(){
    //     var xvalue = $(this).val()
    //     var letters = /^[A-Za-z]+$/;
    //     if(xvalue.value.match(letters)){
    ...
    $( ".appoinment-form" ).submit(function( event ) {
        ...
        var center = $('#your_center').val()
        var date = $('#your_date').val()
        var time = $('#your_time').val()
        var message= $('#textarea_message').val()
        $('#errox').css({'color':'blue','font-size':'18px'}).text('Placing your request please hold')
        if (name && email && phone && center && date && time){
            $('#errox').fadeOut(4000)
            $.ajax({
                type: "GET",
                url: URL+'/api/v1/request/'+name+'/'+email+'/'+phone+'/'+center+'/'+date+'/'+time+'/'+message,
                success: function (data) {
                    if(data.status == 200){
                        $('#errox').css({'color':'green','font-size':'18px'}).html('Request created Successfully<br>We would get back to you soon').fadeOut(12000)
                        $('#your_name').val('')
                        $('#your_email').val('')
                        $('#your_phone').val('')
                        $('#your_center').val('')
                        ...
                        $('#errox').css({'color':'red','font-size':'18px'}).text('Request count not be created..Try again in some minutes').fadeOut(7000)
                    }
                },
                error: function (res) {
                    $('#errox').css({'color':'red','font-size':'18px'}).text('An Error has Occured').fadeOut(7000)
                    console.log(res)
            ...
            $('#errox').empty()
            $('#errox').css({'color':'red','font-size':'18px'}).text('ERROR found').fadeOut(7000)
        }
    })

    $( ".sub_btn" ).submit(function( event ) {
        event.preventDefault();
        ...
        $('.errx').css({'color':'blue','font-size':'18px'}).text('Subscribing...')
        if (email){
            $('.errx').fadeOut(4000)
            $.ajax({
                type: "GET",
                url: URL+'/api/v1/sub/'+email,
                success: function (data) {
                    if(data.status == 200){
                        $('.errx').css({'color':'green','font-size':'18px'}).text('Subscription Successful').fadeOut(12000)
                        $('#mc4wp_email').val('')
                    } else {
                        $('.errx').css({'color':'red','font-size':'18px'}).text('Subscription Failed').fadeOut(7000)
                    }
                },
                error: function (res) {
                    $('.errx').css({'color':'red','font-size':'18px'}).text('An Error has Occured').fadeOut(7000)
                    console.log(res)
            ...
            $('.errx').empty()
            $('.errx').css({'color':'red','font-size':'18px'}).text('ERROR found').fadeOut(7000)
        }
    })
    ...
    if ($(".section-custom-bg").length) {
        ...
        $(".section-custom-bg").each(function () {
            ...
            var bg_img = "/home/slide/1615791573685.jpg",
                    bg_color = "#000000",
                    bg_opacity = "0.5",
                    bg_color_2 = "#000000",
                    bg_opacity_2 = "0.8",
    ...
    if ($('#contact-form').length) {
        ...
        $contact_form.find("button[type=submit]").on("click", function () {
            ...
            if (contact_form_bValid === true) {
                ...
                $.ajax({
                    type: "GET",
                    url: URL+'/api/v1/sayHello/'+user_name+'/'+user_email+'/'+email_subject+'/'+email_message,
                    success: function (data) {
                        ...
                        if (data.status === 200) {
                            ...
                            $('.sentt').css({'color':'blue', 'font-size':'16px'}).text('Please wait ...')

                            setTimeout(function () {
                                $('.sentt').empty()
                                $('.sentt').css({'color':'green', 'font-size':'16px'}).text('SENT').fadeOut(7500)
